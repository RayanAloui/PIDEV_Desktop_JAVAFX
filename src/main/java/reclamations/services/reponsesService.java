...
{Connection cnx;
    static reponsesService instance;
    ...
    // Méthode pour obtenir l'instance unique de reponsesService
    public static reponsesService getInstance() {
        if (instance == null) {
            instance = new reponsesService();
        }
        return instance;
    }

    @Override
    public void ajouter(reponses reponse) {
    ...
    @Override
    public void modifier(reponses reponse) {
        // Validation des données
        if (reponse.getDescription() == null || reponse.getDescription().trim().length() < 10 ||
                reponse.getDate() == null ||
                reponse.getStatut() == null || (!reponse.getStatut().equals("Pending") &&
