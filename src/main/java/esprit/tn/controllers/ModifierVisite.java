...
public class ModifierVisite {
    ...
    public void setVisite(visites visite) {
        this.visite = visite;
        visiteur.setText(String.valueOf(visite.getId_visiteur())); // ID du visiteur
    ...
    void ModifierV(ActionEvent event) {
        try {
            ...
            if (dateValue == null || motifValue.isEmpty() || visiteurValue.isEmpty()) {
    ...
    void retour(ActionEvent event) {
        try {
            ...
            int idv=Integer.parseInt(visiteur.getText());
            AfficherVisite controller = loader.getController();
            controller.initialize(idv);
            controller.IDV=idv;
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
