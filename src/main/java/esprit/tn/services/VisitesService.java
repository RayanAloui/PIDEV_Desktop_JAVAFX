...
public class VisitesService implements Iservices<visites> {
    ...
    public List<visites> getVisitesByVisiteur(int visiteurId) {
        List<visites> liste = new ArrayList<>();
        try {
            String req = "SELECT * FROM visites WHERE id_visiteur = ?";
            ...
            System.out.println("Erreur lors de la récupération des visites : " + ex.getMessage());
        }
        return liste;
    }

    public void SendSMS(String phoneNumber, LocalDate date, LocalTime time) {
        try {
            String apiKey = "8351129"; // Remplace par ta clé API CallMeBot
            String message = "Votre rendrez-vous à été fixé pour le :  "+ date + " à "+time+" :)";
            String encodedMessage = URLEncoder.encode(message, "UTF-8");

            String urlString = "https://api.callmebot.com/whatsapp.php?phone=+216" + phoneNumber + "&text=" + encodedMessage + "&apikey=" + apiKey;
